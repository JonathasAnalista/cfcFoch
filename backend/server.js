...
async function activateProFor(uid, purchase) {
...
async function activateProByEmail(email, purchase, days = 365) {
  if (!email) return;
...
async function activateProLegacyUser(uid, purchase, days = 365) {
  if (!uid) return;
  const ref = db.collection("users").doc(uid);
  await db.runTransaction(async (tx) => {
    const snap = await tx.get(ref);
  ...
  try { await admin.auth().setCustomUserClaims(uid, { plan: "pro" }); } catch {}
}
...
app.post("/api/mp/create-preference", async (req, res) => {
  try {
    ...
    return res.json({ init_point: r.data.init_point });
  } catch (e) {
...
app.post("/api/mp/webhook", async (req, res) => {
  try {
    ...
    if (isApproved && isBRL && (md?.email || md?.uid) && amount >= (md.amount || 0)) {
      ...
      if (md.email) {
        await activateProByEmail(md.email, purchase);
      }
      await activateProLegacyUser(md.uid, purchase);
    } else {
